...
let SendEnvelope = function _SendEnvelope(dsJwtAuth) {
  // private globals
...
/**
  ...
  */
SendEnvelope.prototype.sendEnvelope1 = async function _sendEnvelope1(args){
  // The checkToken method is asynchronous. So we use the `await` instruction
  // to access the promise's result.
  await this._dsJwtAuth.checkToken();
  let env = this._create_envelope_1(args)
    , envelopesApi = new docusign.EnvelopesApi(this._dsApi)
      // Currently, the DocuSign Node SDK does not provide a promise interface.
      // The helper method _dsJwtAuth.make_promise returns a promisfied version
      // of the SDK method. 
    , createEnvelope_p = this._dsJwtAuth.make_promise(envelopesApi, 'createEnvelope')
    , accountId = this._dsJwtAuth.get_accountId()
      // Using `await` to access the promise's result:
    , results = await createEnvelope_p(accountId, {envelopeDefinition: env});
  return results
}
 ...
 * @private
 */
SendEnvelope.prototype._create_envelope_1 = function __create_envelope_1(args){
...
SendEnvelope.prototype._envelope_1_document_1 = function __envelope_1_document_1(args) {
  return `
  <!DOCTYPE html>
  <html>
      <head>
...
module.exports = SendEnvelope;  // SET EXPORTS for the module.
