...
const moment = require('moment')
  ...
  // private globals
  let objVariables = {};
  objVariables.tokenReplaceMin = 10; // The token must expire at least this number of
                               // minutes later or it will be replaced
  objVariables.jwtLifeSec = 3600; // requested lifetime for the token is 1 hour
                           // (the max available from DocuSign)
  objVariables.jwtLifeSec = 3600; // requested lifetime for the token is 1 hour
  objVariables.token = null;        // The bearer token. Get it after calling (await #checkToken)
  objVariables.tokenExpirationTimestamp = null;  // when does the token expire?
  objVariables.user = {}; // object with user data from the UserInfo method
  objVariables.accountId = null; // current account
  objVariables.accountName = null; // current account's name
  objVariables.baseUri = null; // eg https://na2.docusign.net
  objVariables.dsApi = null; // the docusign sdk instance
  objVariables.debug = null;
 ...
 * @function
 * @param owningObj An object that has method method_name
 * @param method_name The string name of the existing method
 * @returns {promise} a promise version of the <tt>method_name</tt>.
 */
DS_JWT_Auth.prototype.make_promise = function _make_promise(owningObj, method_name){
  let promise_name = method_name + '_promise';
  // A simple cache: If the promise version of the method is not cached
  // Then create/cache it for next time.
  if (!(promise_name in owningObj)) {
    owningObj[promise_name] = promisify(owningObj[method_name]).bind(owningObj)
  }
  return owningObj[promise_name]
}
