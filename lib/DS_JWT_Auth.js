...
let DS_JWT_Auth = function _DS_JWT_Auth(debug) {
  ...
  this._token = null;        // The bearer token. Get it after calling (await #check_token)
...
DS_JWT_Auth.prototype.checkToken = async function _checkToken() {
  ...
  if (this._debug) {
    ...
    //if (!needToken) {this._debug_log('check_token: Using current token')}
  }
...
DS_JWT_Auth.prototype._getToken = async function __getToken() {
  ...
  return (
    ...
    .then (result => {
      ...
      this._debug_log(`check_token: Token received! Expiration: ${this._tokenExpirationTimestamp.format()}`);
      return Promise.resolve()
    })
  )
}
...
DS_JWT_Auth.prototype._getAccount = async function _getAccount(targetAccountId){
  ...
  let url = `${dsConfig.authentication_url}/oauth/userinfo`;
...
/**
 * Returns an aud from the authentication_url setting
 * @function
 */
DS_JWT_Auth.prototype._aud = function(){
  let authentication_url = dsConfig.authentication_url;
  let aud;
  if (authentication_url.indexOf('https://') > -1) {
    aud = authentication_url.substr(8); // Remove first 8 characters
  } else if (authentication_url.indexOf('http://') > -1) {
    aud = authentication_url.substr(7); // Remove first 7 characters
  }
  return aud
};
