...
DsJwtAuth.getToken = async function _getToken() {
  ...
  const results = await docusign.requestJWTUserToken(dsConfig.clientId,
          dsConfig.impersonatedUserGuid, scopes, dsConfig.privateKey,
          jwtLifeSec);
  const expiresAt = moment().add(results.expires_in, 's');
  return {accessToken: results.access_token, tokenExpirationTimestamp: expiresAt};
}
...
DsJwtAuth.getUserInfo = async function _getUserInfo(){
  ...
  ({account_id: DsJwtAuth.accountId,
    account_name: DsJwtAuth.accountName,
    base_uri: DsJwtAuth.basePath} = accountInfo);
